...
Write-Host "✓ Uploaded to GCS" -ForegroundColor Green
Write-Host ""
...
if ($ops) {
    ...
    if ($ops) {
        Write-Host "✗ Timeout waiting for operations to complete" -ForegroundColor Red
        Write-Host "Please try again later or cancel the running operations" -ForegroundColor Yellow
        exit 1
    }
}

Write-Host "✓ No running operations" -ForegroundColor Green
Write-Host ""
...
if ($LASTEXITCODE -ne 0) {
    ...
    if ($importResult -like "*Duplicate entry*" -or $importResult -like "*already exists*") {
        ...
        Write-Host "2. Modify the SQL file to use INSERT IGNORE or ON DUPLICATE KEY UPDATE" -ForegroundColor White
...
if ($LASTEXITCODE -eq 0) {
    Write-Host "✓ GCS file removed" -ForegroundColor Green
} else {
...
Write-Host "✓ SQL file uploaded to GCS" -ForegroundColor Green
Write-Host "✓ Data imported into Cloud SQL" -ForegroundColor Green
Write-Host "✓ Database now contains test data for UI" -ForegroundColor Green
Write-Host ""
Write-Host "Test Data Includes:" -ForegroundColor Yellow
Write-Host "  • 17 Permissions (CRUD operations for all resources)" -ForegroundColor White
...
Write-Host "  • 16 KYC Documents (various statuses)" -ForegroundColor White
...
Write-Host "   - Users list: 12 users" -ForegroundColor Gray
Write-Host "   - Organizations list: 8 organizations" -ForegroundColor Gray
Write-Host "   - KYC documents: 16 documents" -ForegroundColor Gray
Write-Host "   - Questionnaires: 20 questions with 48 answers" -ForegroundColor Gray
Write-Host ""
Write-Host "To verify data, run: .\quick-test-gcp.ps1" -ForegroundColor Cyan
Write-Host ""
